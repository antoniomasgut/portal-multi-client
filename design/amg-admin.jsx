    ...
    <aside className={`${w} shrink-0 bg-[#13132a] border-r border-[rgba(255,107,0,0.12)] flex flex-col transition-all`}>
      ...
      <div className={`border-t border-[rgba(255,107,0,0.12)] ${collapsed ? 'p-3' : 'p-4'}`}>
        <div className={`flex items-center gap-3 ${collapsed ? 'justify-center' : ''}`}>
          <div className="w-9 h-9 bg-gradient-to-br from-[#FF6B00] to-[#FF9A3C] btn-clip flex items-center justify-center text-black font-bold text-xs shrink-0">MA</div>
          {!collapsed && (
            <div className="flex-1 min-w-0">
              <div className="text-sm font-semibold truncate">Marc Alonso</div>
              <div className="flex items-center gap-1.5 mt-0.5">
                <AMGBadge tone="accent" className="!h-4 !text-[9px] !px-1.5">ADMIN</AMGBadge>
                <span className="f-mono text-[10px] text-[#64748b] truncate">marc@amg.cat</span>
              </div>
            </div>
          )}
        </div>
