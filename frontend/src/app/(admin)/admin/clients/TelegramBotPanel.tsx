...
import { useTelegramBot, useSaveTelegramBot, useGenerateTelegramBot, FAQ, TelegramBotConfig } from '../../../../hooks/useTelegramBot'
...
export default function TelegramBotPanel({ clientId, companyName }: { clientId: string; companyName: string }) {
  const { data, isLoading } = useTelegramBot(clientId)
  const save     = useSaveTelegramBot(clientId)
  const generate = useGenerateTelegramBot(clientId)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between card">
        <div>
          ...
          <p className="text-gray-500 text-xs">El bot respondrà als missatges de Telegram</p>
        </div>
        <button
      ...
      <div className="space-y-4">
        ...
        <div>
          <label className="form-label">Token del Bot (BotFather)</label>
          <input
            ...
            type="password"
          />
          <p className="text-gray-500 text-xs mt-1">Obtén el token creant un bot a @BotFather de Telegram.</p>
        </div>

        <div>
          <label className="form-label">Missatge de benvinguda</label>
          <textarea className="form-input" rows={3} value={greeting} onChange={e => setGreeting(e.target.value)} placeholder="Hola! Sóc el teu assistent virtual de Telegram…" />
        </div>
