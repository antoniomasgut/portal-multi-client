...
export default function InvoicesPage() {
  ...
  const STATUS_LABELS: Record<string, string> = {
    DRAFT: t('invoices.status_draft'), PENDING: t('invoices.status_pending'),
    ...
    <div className="p-6 max-w-6xl mx-auto animate-fade-in-up">
      ...
      <div className="flex justify-between items-start mb-8">
        <div>
          ...
          <h1 className="font-orbitron font-black text-3xl text-[#FF6B00]">{t('invoices.title')}</h1>
          {!isLoading && (
            <p className="font-mono text-[11px] text-[var(--text-muted)] mt-1 tracking-widest">
              {t(invoices.length === 1 ? 'invoices.invoice_count_one' : 'invoices.invoice_count_other', { count: invoices.length })}
            </p>
          )}
        </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            { label: t('invoices.stat_total_paid'), value: `${stats.totalPaid.toFixed(2)}€`, color: 'border-l-[#4ade80]', text: 'text-[#4ade80]' },
            { label: t('invoices.stat_this_month'),  value: `${stats.thisMonth.toFixed(2)}€`, color: 'border-l-[#60a5fa]', text: 'text-[#60a5fa]' },
            { label: t('invoices.stat_pending'),     value: stats.pendingCount,               color: 'border-l-[#FF6B00]', text: 'text-[#FF6B00]' },
            { label: t('invoices.stat_overdue'),     value: stats.overdueCount,               color: 'border-l-[#ff4444]', text: 'text-[#ff4444]' },
          ].map(s => (
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-[var(--bg-1)] border border-[var(--border)] w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)]">
              <div>
                <p className="font-mono text-[9px] tracking-[4px] text-[#FF6B00] uppercase">{t('invoices.modal_tag')}</p>
                <p className="font-rajdhani font-semibold text-[var(--text)] mt-0.5">{t('invoices.modal_subtitle')}</p>
              </div>
            ...
            <form onSubmit={handleGenerate} className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-1">
                <button type="submit" className="btn-primary text-xs" disabled={generate.isPending || !genClientId}>
                  {generate.isPending ? t('invoices.form_btn_generating') : t('invoices.form_btn_generate')}
                </button>
